/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { VisitorWhereUniqueInput } from "./VisitorWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { VisitorUpdateInput } from "./VisitorUpdateInput";

@ArgsType()
class UpdateVisitorArgs {
  @ApiProperty({
    required: true,
    type: () => VisitorWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => VisitorWhereUniqueInput)
  @Field(() => VisitorWhereUniqueInput, { nullable: false })
  where!: VisitorWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => VisitorUpdateInput,
  })
  @ValidateNested()
  @Type(() => VisitorUpdateInput)
  @Field(() => VisitorUpdateInput, { nullable: false })
  data!: VisitorUpdateInput;
}

export { UpdateVisitorArgs as UpdateVisitorArgs };

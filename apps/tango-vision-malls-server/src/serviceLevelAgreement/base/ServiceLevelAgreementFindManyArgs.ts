/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ServiceLevelAgreementWhereInput } from "./ServiceLevelAgreementWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { ServiceLevelAgreementOrderByInput } from "./ServiceLevelAgreementOrderByInput";

@ArgsType()
class ServiceLevelAgreementFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ServiceLevelAgreementWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => ServiceLevelAgreementWhereInput, { nullable: true })
  @Type(() => ServiceLevelAgreementWhereInput)
  where?: ServiceLevelAgreementWhereInput;

  @ApiProperty({
    required: false,
    type: [ServiceLevelAgreementOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [ServiceLevelAgreementOrderByInput], { nullable: true })
  @Type(() => ServiceLevelAgreementOrderByInput)
  orderBy?: Array<ServiceLevelAgreementOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ServiceLevelAgreementFindManyArgs as ServiceLevelAgreementFindManyArgs };

/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ContactLogWhereUniqueInput } from "./ContactLogWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { ContactLogUpdateInput } from "./ContactLogUpdateInput";

@ArgsType()
class UpdateContactLogArgs {
  @ApiProperty({
    required: true,
    type: () => ContactLogWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => ContactLogWhereUniqueInput)
  @Field(() => ContactLogWhereUniqueInput, { nullable: false })
  where!: ContactLogWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => ContactLogUpdateInput,
  })
  @ValidateNested()
  @Type(() => ContactLogUpdateInput)
  @Field(() => ContactLogUpdateInput, { nullable: false })
  data!: ContactLogUpdateInput;
}

export { UpdateContactLogArgs as UpdateContactLogArgs };
